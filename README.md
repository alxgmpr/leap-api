...
- **Two tiers.** As of the unverified import, `dist/openapi.yaml` carries two
  kinds of content and every consumer must tell them apart. The **hand-refined
  tier** (211 paths, 344 schemas) is checked against captured traffic. The
  **imported tier** (163 paths, 80 schemas) is the firmware route table taken
  as-is: no capture has exercised it, no platform availability is known, and
  its shapes are the generator's staging output. Everything in the imported
  tier carries `x-leap-verified: false`, on the path item, on every operation,
  and on every schema, and is labelled *unverified* everywhere the site shows
  it. `npm run coverage` and `npm run fields` report the refined tier's numbers
  and count the import separately, so no figure below changed meaning when the
  import landed. Only routes whose path can be read at face value were
  imported; see "Paths in doubt" below. The allowlist is committed at
  `spec/unverified-paths.json` and regenerated by `npm run derive:unverified`.
- **Paths in doubt.** 51 firmware routes are **not** represented at all, and
  cannot be without guessing. Each begins with another resource's name, so it
  may be a concatenation the extraction mangled -- `/areastatus` for
  `/area/status` -- rather than a path in its own right. That defect is proven:
  six routes, including `/devicestatus` and
  `/device/{id}/buttongroupexpanded`, were confirmed to be the slashed form and
  are documented under it. Nothing separates the remaining 51 from genuine
  resources without asking hardware. Importing one would mean guessing the path
  before guessing the shape.
- **Paths.** The firmware route extraction recovered 410 distinct route
  templates. 228 of those 410 (56%) are **absent from the hand-refined
...
npm run derive:unverified  # regenerate spec/unverified-paths.json from the
                       # face-value classification
